...
export async function analyzeColumnSentiment(
  ...
  if (!languageResponse || !languageResponse.ResultList || (languageResponse.ErrorList?.length ?? 0 > 0))
...
const dominantLanguage = Object.keys(languageCounts).reduce((a, b) => languageCounts[a] > languageCounts[b] ? a : b) as LanguageCode;
//console.log(dominantLanguage);
  ...
  if (!sentimentResponse || !sentimentResponse.ResultList || (sentimentResponse.ErrorList?.length ?? 0 > 0))
  ...
  // Extract the sum of all sentiment scores to calculate the average for every category and the final sentiment, which is the greatest number
  ...
  const sumSentimentScores = sentimentScores.reduce((sum, score) => {
    if(!sum || !score) return
    return {
  ...
  const sentimentResult = Object.keys(averageSentimentScores).reduce((a, b) => averageSentimentScores[a] > averageSentimentScores[b] ? a : b);
